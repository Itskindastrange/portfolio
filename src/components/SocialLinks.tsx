...
export default function SocialLinks() {
  return (
    <>
      ...
      <motion.div 
        className="fixed right-4 top-1/2 transform -translate-y-1/2 z-50 hidden md:flex flex-col gap-4"
      ...
      {/* Mobile Social Links */}
      <motion.div 
        className="fixed bottom-4 left-0 right-0 z-50 md:hidden"
        ...
        <div className="flex justify-center gap-4 px-4">
          {socialLinks.map((link, index) => (
            <motion.a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className={`p-3 bg-black/30 backdrop-blur-sm rounded-full text-white/60 ${link.color} transition-all duration-300`}
              whileTap={{ scale: 0.95 }}
